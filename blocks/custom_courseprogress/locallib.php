...
function custom_process_html($html, $instanceid) {

    // Replacing the CDN URL;
    $html = custom_replace_url($html);
    $html = custom_replace_instance($html, $instanceid);
    return $html;
}

function custom_process_css($css, $instid) {

    $css = custom_replace_cdn_url($css);

    return custom_get_wrapped_css($css, $instid);
}
function custom_replace_instance($content, $instanceid) {
    return str_replace("[[inst]]", $instanceid, $content);
}

function custom_replace_url($content) {
    // Replacing the CDN URL;
...
function custom_get_maximum_active_user_in_course($limit = 10) {
    global $DB;
    $last30daystimestamp = strtotime('-30 days');
    $sql = "SELECT count(DISTINCT mls.userid) as usercount, mls.courseid
    FROM  {user} mu 
    JOIN {logstore_standard_log} mls ON mu.id = mls.userid WHERE mls.`action`= 'viewed' AND mu.deleted = 0 AND mls.courseid NOT IN (0,1)
...
function get_course_access_usersid($courseid, $days, $userid = null) {
    global $DB;
    $timestamp30DaysAgo = strtotime("-$days days");

    if ($userid) {
        $userdetail = $DB->get_record_sql('SELECT ul.id, c.fullname, ul.timeaccess, c.id as courseid FROM {user_lastaccess} ul JOIN {user} u ON u.id = ul.userid JOIN {course} c ON c.id = ul.courseid WHERE ul.userid = '. $userid .' AND u.deleted = 0 AND ul.timeaccess > ' . $timestamp30DaysAgo . ' and courseid = ' . $courseid . '');
        return [
        ...
        $userdetail = $DB->get_records_sql('SELECT ul.id, ul.userid FROM {user_lastaccess} ul JOIN {user} u ON u.id = ul.userid WHERE u.deleted = 0 AND ul.timeaccess > ' . $timestamp30DaysAgo . ' and courseid = ' . $courseid . '');
