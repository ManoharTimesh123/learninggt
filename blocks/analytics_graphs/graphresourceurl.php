    ...
    if ($resourceid == 0) { /* First time in loop -> get topic and content name */
        ...
        if ($resourceid != $tuple->ident) {
            ...
            if ($tuple->userid) {
                $statistics[$counter]['studentswithaccess'][] = array(
                    'userid' => $tuple->userid,
                    'username' => $tuple->firstname . " " . $tuple->lastname,
                    'email' => $tuple->email,
...
if (strlen($topic) && $state && $dataformat) {
    ...
    if ($dataformat) {
        ...
        $filename = clean_filename('testing');
        ...
        \core\dataformat::download_data(
            $filename,
            $dataformat,
            array_merge($originfields, $profilefields),
            $iterator,
...
<body>
    ...
    <script>
        ...
        $.each(geral, function(index, value) {
            ...
            <?php
            $params['userstate'] = 'studentswithaccess';
            ...
            if (typeof value.studentswithaccess != 'undefined') {
                ...
                div += "<div class='div_nomes' id='" + index + "-" +
                    ...
                    "'>" + titulo + " <br><button onclick = 'window.location.href = \"<?= $url ?>&topic=" + topic + "&dataformat=csv\"'> Download CSV</button> <br> <button onclick = downloadxls()> Download CSV</button></div>";
            }
