...
require_once($CFG->dirroot . "/blocks/custom_pastview/renderer.php");
class block_custom_pastview extends block_base {
    public function init() {
        $this->title = get_string('custom_pastview', 'block_custom_pastview');
    }
    public function get_content() {
        global $OUTPUT, $CFG;
        $this->content = new stdClass;
        $html = render_custom_pastview();
        $templatecontext = [];
        $templatecontext['blockstyle'] = $CFG->wwwroot . '/blocks/custom_pastview/style.css';
        ...
        $this->content->text .= $OUTPUT->render_from_template('block_custom_pastview/blockcontent', $templatecontext);
