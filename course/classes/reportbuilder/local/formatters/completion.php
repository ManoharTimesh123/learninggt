...
class completion {
    ...
    public static function completion_progress(?string $value, stdClass $row): string {
        ...
        $progress = $DB->get_record_sql("SELECT up.progress FROM {local_user_progress} up WHERE up.userid = :userid AND up.courseid = :courseid", [
            'userid' => $row->userid,
            'courseid' => $courseid,
        ]);
        if (empty($progress)) {
    ...
    // code by manohar
    public static function result(?string $value, stdClass $row): string {
        global $CFG, $DB;
        require_once($CFG->libdir . '/completionlib.php');
        ...
        $courseid = (int) $row->courseid;
        $result = $DB->get_record_sql("SELECT la.result FROM {local_acca} la WHERE la.userid = :userid AND la.courseid = :courseid", [
            'userid' => $row->userid,
            'courseid' => $courseid,
        ]);
        if (empty($result)) {
